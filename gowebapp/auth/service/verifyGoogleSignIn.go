...
func (as *AuthService) VerifyGoogleSigninAndGetProfile(req *model.SignInRequest) (*model.SignInResponse, error) {
	log.Println("inside google sign in and get profile")
	var err error
	log.Println(req)
	...
	if err != nil {
		log.Println(err)
		response := model.SignInResponse{IsSuccessful: false, Code: 2, Message: []string{"illegal idToken"}}
		return &response, err
	}
	...
	authType := dbConstants.Custom
	providerType := dbConstants.DBGoogle
		...
		if errors.Is(err, respond.ErrNoRecord) {
			...
			if err != nil {
				if errors.Is(err, respond.ErrNoRecord) {
					...
					response := model.SignInResponse{IsSuccessful: true, Code: 2, UserBody: bodyPayload, BearerToken: "bearer_" + dbUser.Base.ID.String()}
					return &response, nil
			...
			response := model.SignInResponse{IsSuccessful: true, Code: 2, UserBody: bodyPayload, BearerToken: "bearer_" + dbEmail.UserId.String()}
			return &response, nil
