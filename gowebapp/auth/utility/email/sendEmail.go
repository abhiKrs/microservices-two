...
func SendEmailviaAws(email string, token *string, messageType constants.EmailType, linkType *dbConstants.DBMagicLinkType, successType *constants.SuccessType, cfg *config.Config) error {
	var Sender = cfg.Api.MAILER_EMAIL
	// log.Println(cfg.Api.MAILER_EMAIL)
	var templateData = TemplateBody{}
	if messageType == constants.MagicLink {
