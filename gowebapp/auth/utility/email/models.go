...
func (tb *TemplateBody) MagicLinkTemplate(linkType dbConstants.DBMagicLinkType, redirectLink string) {
	if linkType == dbConstants.Register {
	...
	} else if linkType == dbConstants.Login {
		...
		tb.URL = redirectLink
		tb.Footnote = "This link is provided for sigle time use only. The link will expire in 1 hour."
	} else if linkType == dbConstants.ResetPassword {
