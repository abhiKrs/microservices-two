...
func (au *AuthDataAccess) CreateNewUserAndSendMagicLink(emailNew string) (*schema.Email, *schema.MagicLink, error) {
	...
	err = au.db.Transaction(func(tx *gorm.DB) error {
		...
		// res := email.SendEmailviaAws(recepient, &tokenStr, constants.MagicLinkMessage, &linkType, nil, au.cfg)
		// res := email.SendEmailviaDapr(recepient, &tokenStr, constants.MagicLinkMessage, &linkType, nil)
		res := email.SendEmail(recepient, &tokenStr, constants.MagicLinkMessage, &linkType, nil)
		if res != nil {
			log.ErrorLogger.Println(res.Error())
			// return res
		}
		return nil
	})
...
func (au *AuthDataAccess) ResendAuthLink(dbEmail *schema.Email, linkType dbConstants.DBMagicLinkType) (*schema.MagicLink, error) {
	...
	err := au.db.Transaction(func(tx *gorm.DB) error {
		...
		// errr = email.SendEmailviaDapr(recipient, &tokenStr, constants.MagicLinkMessage, &linkType, nil)
		errr = email.SendEmail(recipient, &tokenStr, constants.MagicLinkMessage, &linkType, nil)
		if errr != nil {
			log.ErrorLogger.Println(errr)
			// return errr
		}
		return nil
...
func (au *AuthDataAccess) VerifyRegisterLinkAndCreateProfile(dbMagicLink *schema.MagicLink) (*schema.Profile, string, error) {
	...
	err := au.db.Transaction(func(tx *gorm.DB) error {
		...
		dbTeam = &teamSchema.Team{CreatedBy: dbProfile.ID, Name: "My Team"}
		result = tx.Create(&dbTeam)
		if result.Error != nil {
			log.ErrorLogger.Println(result.Error)
			return result.Error
		}
...
func (au *AuthDataAccess) VerifyLoginLinkAndGetProfile(dbMagicLink *schema.MagicLink) (*schema.Profile, error) {
	...
	if !dbEmail.Verified {
		log.InfoLogger.Println("called verifyLogin when email not verified!!!!")
...
func (au *AuthDataAccess) VerifyPasswordLink(dbMagicLink *schema.MagicLink) (*schema.MagicLink, error) {
	...
	if !dbEmail.Verified {
		log.WarningLogger.Println("called verifyLogin when email not verified!!!!")
