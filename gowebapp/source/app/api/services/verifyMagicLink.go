...
func (as *AuthService) VerifyMagicLink(req *models.SignInRequest) (*models.SignInResponse, error) {
	...
	case uint8(dbConstants.Register.EnumIndex()):
		...
		response := models.SignInResponse{IsSuccessful: true, Code: int(dbMagicLink.LinkType), Email: dbEmail.Email, UserBody: bodyPayload, BearerToken: "bearer_" + dbMagicLink.UserId.String()}
